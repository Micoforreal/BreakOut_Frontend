...
const ProfileModal = ({showProfileModal, setShowProfilModal}) => {
    const {start } = createContext(userContext)
    const {connected, publicKey} = useWallet()

   
    const {createAccount, createCharacter} = useHoneyComb()
        const {
        control,
        handleSubmit,
        formState: { errors },
        ...
        if (connected === true) {
            ...
            const response = await createAccount({fullName:data.fullName, user:publicKey})
