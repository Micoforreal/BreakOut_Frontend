...
import menuMusicFile from './assets/sounds/menuMusic.mp3';
  ...
  async LoadAssets() {
    ...
    promises.push(this.AddAsset(menuMusicFile, audioLoader, "menuMusic"));
  ...
  EntitySetup() {
    ...
    const gunparam = {
      camera: this.camera,
      model: this.assets["ak47"].scene,
      flash: this.assets["muzzleFlash"],
      world: this.physicsWorld,
      shotSoundBuffer: this.assets["ak47Shot"],
      listner: this.listener,
    };
    ...
    playerEntity.AddComponent(new Weapon(gunparam));
