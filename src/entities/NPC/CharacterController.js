...
export default class CharacterController extends Component{
    ...
    this.healthBarBG = new THREE.Mesh(bgGeom, backgroundMat);

    // Foreground (actual health)
    const fgGeom = new THREE.PlaneGeometry(barWidth, barHeight);
    this.healthBarFG = new THREE.Mesh(fgGeom, foregroundMat);
    this.healthBarFG.position.z = 0.01; // Slight offset to prevent z-fighting

    // Combine in a group
    this.healthBar = new THREE.Group();
    this.healthBar.add(this.healthBarBG);
    this.healthBar.add(this.healthBarFG);

    this.healthBar.scale.set(0.5, 0.5, 0.5); // Optional scaling
...
UpdateHealthBar() {
    ...
    const headHeight = 2.2; // Adjust to your model’s height
    this.healthBar.position.copy(this.model.position).add(new THREE.Vector3(0, headHeight, 0));
    ...
    // const camPos = this.scene.useData.camera.postion.clone();

    // camPos.y = this.healthBar.position.y
    this.healthBar.lookAt(this.tempVec);

    // console.log(this.scene)
    
    // const cam
    // Update foreground bar scale based on health
    const healthRatio = this.health / 100;
    this.healthBarFG.scale.x = healthRatio;
    this.healthBarFG.position.x = -(1 - healthRatio) * 0.5; // Keep left-aligned
}
